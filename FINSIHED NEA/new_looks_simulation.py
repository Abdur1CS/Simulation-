...
# List of countries and their parameters
country_parameters = {
...
def plotting(comparisonData, params_list): 
    ...
    # Add a text box for final values inside the graph
    ...
    # here change the box shape so it's not in the center anymore.
    plt.show()
...
def start_simulation(single_epidemic=True):
    ...
    if single_epidemic:
        ...
        # Adjust population size without overriding other parameters
        params['population_size'] += country_parameters[selected_country]['population_size']  # Add population size
        ...
        for epidemic_name in [epidemic_name1, epidemic_name2]:
            params = get_params(epidemic_name)
            params['population_size'] += country_parameters[country_choice.get()]['population_size']  # Add population size
            population = makingThepopulation(params)
            data = simulationOfTheloop(params, population)
            comparisonData.append(data)
            params_list.append(params)
...
root.configure(bg='#f0f0f0')  # Updated background color

# Title Label
title_label = tk.Label(root, text="Epidemic Simulation", font=("Arial", 24), bg='#f0f0f0', fg='#333')  # Updated font style and color
title_label.pack(pady=20)

# Frame for country selection
country_frame = tk.Frame(root, bg='#f0f0f0')
country_frame.pack(pady=10)

ttk.Label(country_frame, text="Select Country:").grid(column=0, row=0, padx=10, pady=5)
country_choice = ttk.Combobox(country_frame, values=list(country_parameters.keys()))
country_choice.grid(column=1, row=0, padx=10, pady=5)

# Frame for single simulation
single_frame = tk.Frame(root, bg='#f0f0f0')
single_frame.pack(pady=10)

ttk.Label(single_frame, text="Select Epidemic for Single Simulation:").grid(column=0, row=0, padx=10, pady=5)
epidemic_choice = ttk.Combobox(single_frame, values=list(epidemics.keys()))
epidemic_choice.grid(column=1, row=0, padx=10, pady=5)
ttk.Button(single_frame, text="Simulate", command=lambda: start_simulation(True)).grid(column=2, row=0, padx=10, pady=5)

# Frame for comparison
comparison_frame = tk.Frame(root, bg='#f0f0f0')
comparison_frame.pack(pady=10)

ttk.Label(comparison_frame, text="Compare Epidemics:").grid(column=0, row=0, padx=10, pady=5)
epidemic_choice1 = ttk.Combobox(comparison_frame, values=list(epidemics.keys()))
epidemic_choice1.grid(column=1, row=0, padx=10, pady=5)
epidemic_choice2 = ttk.Combobox(comparison_frame, values=list(epidemics.keys()))
epidemic_choice2.grid(column=2, row=0, padx=10, pady=5)
ttk.Button(comparison_frame, text="Compare", command=lambda: start_simulation(False)).grid(column=3, row=0, padx=10, pady=5)

# Frame for custom epidemic
custom_frame = tk.Frame(root, bg='#f0f0f0')
custom_frame.pack(pady=10)

ttk.Button(custom_frame, text="Create Custom Epidemic", command=create_custom_epidemic).grid(column=0, row=0, padx=10, pady=10)
